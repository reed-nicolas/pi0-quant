...
class IPTLinearRTLFunction:
    """
    Functional adapter around the 32×32 Inner Product Trees MXU with
    parametrized accumulator width.

    extra_bits controls accumulator precision:
      int_width = 8 + 7 + extra_bits = 15 + extra_bits
    ...
    def __call__(
        ...
        num_k_tiles = (in_features + vec_len - 1) // vec_len
        log.info(
            "__call__: x%s  w%s  bias=%s  batch=%d  in=%d  out=%d  k_tiles=%d"
            "  scale_exp=%d  int_width=%d",
            tuple(x_q.shape), tuple(w_q.shape), "yes" if b_q is not None else "no",
            batch, in_features, out_features, num_k_tiles, scale_exp, int(self.int_width),
        )
        ...
        log.info(
            "__call__: done  out%s  fmt=%s  int_width=%d",
            tuple(y.shape), self.out_fmt_sel.name, int(self.int_width),
        )
