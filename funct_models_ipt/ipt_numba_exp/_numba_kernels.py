"""Numba-JIT hot path for InnerProductTreesModel with parametrized accumulator width.

Extends ipt_numba by adding _n-suffix variants of every accumulator helper that
accept int_width as a runtime parameter, plus compute_lanes_batch_n which wires
them together.

Formula:  int_width = 8 + 7 + extra_bits = 15 + extra_bits
  extra_bits=0  → int_width=15  (minimum precision)
  extra_bits=17 → int_width=32  (identical to ipt_numba / full hardware precision)
"""
...
# ---------------------------------------------------------------------------
# Original helpers – hardcoded int_width=32 (kept for reference / compatibility)
# ---------------------------------------------------------------------------

@njit(cache=True, inline="always")
def _wrap32(v: np.int64) -> np.int64:
    """Wrap to signed 32-bit (int_width=32)."""
...
# ---------------------------------------------------------------------------
# Parametrized helpers – runtime int_width
# ---------------------------------------------------------------------------

@njit(cache=True, inline="always")
def _wrap_n(v: np.int64, int_width: np.int32) -> np.int64:
    """Wrap to signed int_width-bit integer."""
    mask     = (np.int64(1) << np.int64(int_width)) - np.int64(1)
    sign_bit = np.int64(1) << np.int64(int_width - np.int32(1))
    v = v & mask
    if v & sign_bit:
        v -= (np.int64(1) << np.int64(int_width))
    return v


@njit(cache=True, inline="always")
def _prod_to_aligned_n(prod_bits: np.int32, anchor: np.int32, int_width: np.int32) -> np.int64:
    ...
    sign     = (prod_bits >> np.int32(12)) & np.int32(1)
    man      = prod_bits & np.int32(0x7F)
    left_pad = np.int64(int_width - np.int32(8))
    sig_wide = (np.int64(0x80) | np.int64(man)) << left_pad
    rshift   = np.int64(anchor - (exp_bits - np.int32(13)))  # anchor - unb_exp
    if rshift >= np.int64(0):
        shifted = sig_wide >> rshift
    else:
        shifted = sig_wide << (-rshift)
    mag = _wrap_n(shifted, int_width)
    return -mag if sign else mag


@njit(cache=True, inline="always")
def _bias_to_aligned_n(bias_bits: np.int32, anchor: np.int32, int_width: np.int32) -> np.int64:
    ...
    unb_exp  = exp_field - np.int32(7)
    full_sig = np.int64(frac) if exp_field == np.int32(0) else (np.int64(0x8) | np.int64(frac))
    rshift   = np.int32(anchor) - unb_exp - (int_width - np.int32(4))
    mask     = (np.int64(1) << np.int64(int_width)) - np.int64(1)
    if rshift >= int_width:
        mag = np.int64(0)
    elif rshift >= np.int32(0):
...
@njit(cache=True, inline="always")
def _psum_to_aligned_n(psum_bits: np.int32, anchor: np.int32, int_width: np.int32) -> np.int64:
    ...
    unb_exp  = exp_field - np.int32(127)
    full_sig = np.int64(frac) if exp_field == np.int32(0) else (np.int64(0x80) | np.int64(frac))
    rshift   = np.int32(anchor) - unb_exp - (int_width - np.int32(8))
    mask     = (np.int64(1) << np.int64(int_width)) - np.int64(1)
    if rshift >= int_width:
        mag = np.int64(0)
    elif rshift >= np.int32(0):
...
@njit(cache=True, inline="always")
def _aligned_to_bf16_n(int_in: np.int64, anchor: np.int32, int_width: np.int32) -> np.int32:
    ...
    if val == np.int64(0):
        return np.int32(0)
    f = math.ldexp(float(val), int(anchor) - int(int_width - np.int32(1)))
    return _f64_to_bf16_rne(f)


# ---------------------------------------------------------------------------
# Main exported kernels
# ---------------------------------------------------------------------------
...
def compute_lanes_batch(
    ...
    """Process all batch elements for one (k_tile, out_tile) step (int_width=32)."""
    batch   = acts.shape[0]
...
def compute_lanes_batch_n(
    ...
    out_fmt_sel:   np.int32,    # 0=OutBF16, 1=OutE4M3
    lut:           np.ndarray,  # (65536,) int32
    int_width:     np.int32,    # accumulator bit width (15 + extra_bits)
) -> np.ndarray:                # (batch, num_lanes) int32
    """Process all batch elements for one (k_tile, out_tile) step with runtime int_width."""
    batch   = acts.shape[0]
    n_lanes = wbuf0.shape[0]
    vec_len = acts.shape[1]
    ...
    for b_idx in prange(batch):
        ...
        for lane_idx in range(n_lanes):
            ...
            # ── Pass 2: accumulate using parametrized int_width ──────────
            prod_sum = np.int64(0)
            for i in range(vec_len):
...
def warmup(int_width: np.int32 = np.int32(32)) -> None:
